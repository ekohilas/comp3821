...
let $m_k, p_k$ be the machining and poilishing time of the $k^{th}$ item and $m_i, p_i$ be the machining and polishing time of the $i^{th}$ item.

In the case where $p_i < p_k$ and $m_i = m_k$:
If $A$ selects $p_k$ to be machined first, we get:\\
$T_A = m_k + m_i + \max(p_k - m_i, p_i)$
$T_A_1 = m_k + m_i + p_k - m_i$
$T_A_1 = m_k + p_k$
$T_A_2 = m_k + m_i + p_i$
$T_A_2 = 2m_k + p_i$

If $O$ selects $p_i$ to be machined first, we get:\\
$T_O = m_k + m_i + \max(p_i - m_k, p_k)$
$T_O_1 = m_k + m_i + p_k$
$T_O_1 = 2m_k + p_k$
$T_O_2 = m_k + m_i + p_i - m_k$
$T_O_2 = m_k + p_i$
...
If we violate our greedy choice and select $p_i$ to be machined first,
