...
\begin{align}
    P(x) = (a_0 + x^{17}(a_{17} + x^2(a_{19} + x^2(a_{21} + x^2a_{23})))) \\
    Q(x) = (b_0 + x^{17}(b_{17} + x^2(b_{19} + x^2(b_{21} + x^2b_{23}))))
\end{align}
For every pair, we can calculate its multiplication with 3 large number multiplication using the Karatsuba trick.
As there are 4 pairs, we get 12 large number multiplications
...
\section*{Question 5}
For any input $(a_0, a_1, a_2, ..., a_{2^n-1})$, we can describe $a_i$'s new position by converting $i$ to binary and finding the reversed sequence.
